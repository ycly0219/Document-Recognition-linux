...
def _extract_customer_return(response_body, subject="客商"):
    """读取客商或下单方查询回告的状态码和描述。"""
    try:
        response = response_body["Response"]
    except (KeyError, TypeError) as exc:
        raise ValueError(f"{subject}查询回告缺少 Response") from exc
    if not isinstance(response, dict):
        raise ValueError(f"{subject}查询回告 Response 格式异常")
    if "returnCode" not in response:
        raise ValueError(f"{subject}查询回告缺少 returnCode")
    return response.get("returnCode"), _customer_text(
        response.get("returnDesc")
    )


def is_customer_query_success(response_body, subject="客商"):
    """判断客商或下单方查询回告是否成功，支持字符串和数值业务码。"""
    return_code, _ = _extract_customer_return(response_body, subject)
    return return_code == "0000" or (
        type(return_code) in (int, float) and return_code == 0
    )


def extract_customer_records(response_body, subject="客商"):
    """从 QUERYCO 回告中按接口顺序提取客商或下单方记录。"""
    try:
        items = response_body["Response"]["items"]["item"]
    except (KeyError, TypeError) as exc:
        raise ValueError(f"{subject}查询回告缺少 item 明细") from exc
    if items is None:
        items = []
    elif isinstance(items, dict):
        items = [items]
    if not isinstance(items, list):
        raise ValueError(f"{subject}查询回告 item 格式异常")
    if any(not isinstance(item, dict) for item in items):
        raise ValueError(f"{subject}查询回告 item 明细格式异常")
...
def query_customer_records(timeout=10, customer_type="CO"):
    """调用 QUERYCO，按客商类型返回客商或下单方记录。"""
    subject = (
        "下单方"
        if str(customer_type).strip().upper() == "IP"
        else "客商"
    )
    payload = {
        "data": {
    ...
    try:
        ...
        if response.status_code != 200:
            raise RuntimeError(
                f"{subject}查询接口 HTTP {response.status_code}"
            ) from exc
        raise ValueError(f"{subject}查询回告不是有效 JSON") from exc
    if response.status_code != 200:
        try:
            _, return_desc = _extract_customer_return(
                response_body, subject
            )
        except ValueError:
            return_desc = ""
        detail = f"：{return_desc}" if return_desc else ""
        raise RuntimeError(
            f"{subject}查询接口 HTTP {response.status_code}{detail}"
        )
    if not is_customer_query_success(response_body, subject):
        return_code, return_desc = _extract_customer_return(
            response_body, subject
        )
        detail = (
            f"：{return_desc}"
        ...
        raise RuntimeError(f"{subject}查询失败{detail}")
    return extract_customer_records(response_body, subject)


def customer_record_display_rows(records):
    """把客商或下单方记录转换为列表窗口使用的显示行。"""
    return [
