...
- `GE-ORACLE拣货单` 与 `GE-OSCAR拣货单` 在 `订单类型` 后新增 `客商编码`；输入框始终只读，右侧固定显示「选择客商」按钮；仅在任一明细命中医疗器械 SKU 时必填并启用按钮，未命中时禁用按钮并显示 `CONSIGNEEID`
- 医疗器械单据未填写 `客商编码` 时，标签显示为红色加粗的 `客商编码（医疗器械必填）`；通过「查询客商」回填后标签恢复为 `客商编码`，医疗器械警告和命中明细行标红保持不变
...
- 预览表格支持在选中行下方插入空白行；支持选中一行或多行后通过「复制行 / 粘贴行」按钮或 `Ctrl/Cmd+C / Ctrl/Cmd+V` 整行复制/粘贴
- 预览表格的整行复制/粘贴仅作用于当前文件页签，切换页签后复制内容清空，避免跨文件数据混入
- 预览表格支持 `Ctrl/Cmd+Z` 撤销当前文件页签最近 20 步单元格修改、新增、插入、粘贴和删除操作；单元格编辑中按下会取消当前编辑，撤销历史仅在本次程序运行期间保留
...
- 顶部「选择文件并开始处理」右侧提供「继续查询原任务」按钮，仅对 `结果未生成` 页签启用；点击后按原任务继续查询，不重新上传、不重新提交 OCR，续查成功自动刷新为成功
...
- `GE-发票单` 的 LPN/Serial 与 QTY 不匹配且保留原始行时，将该行标记为拆分异常明细；明细预览整行标红，并在「明细」标题行提示「包含异常明细，请注意」，与医疗器械提示同时存在时合并展示；人工修改异常行后保留标记，删除后清除，撤销可恢复，人工新增或粘贴的行不继承标记；该标记仅用于预览，不影响 Excel 导出、WMS 报文或发送校验
...
- 接口发送二级窗口底部「确认发送」左侧及主界面底部均提供「新增产品」；点击后在独立模态窗口录入产品主数据并通过一次 Flux WMS `putSKU` 请求批量新增到 `GEHC`、`GEHC-BF`、`GEHC-DBY`、`GEHC-ZLKC` 四个货主，四条产品记录仅 `customerId` 不同；产品编码/产品描述必填且标红加粗，六个产品属性选择框联动是否医疗器械，医疗器械勾选后自动启用并锁定效期控制，序列号控制和批次控制必须且只能选择一项，并开放有效期及日/月/年单位填写；发送成功/失败回告展示在弹窗内，成功后弹窗保留便于连续录入，可「清空」继续新增
- 程序启动时先加载用户主目录中的医疗器械目录缓存，再在后台异步调用 `QUERYMD` 刷新；HTTP 200 时覆盖上次缓存，接口失败时继续使用上次成功缓存，网络查询不阻塞 OCR
- 使用医疗器械 SKU 检查三种模板的明细行：命中时在「明细」标题行居中加粗标红显示「该订单包含医疗器械（标红显示），请注意！」，并将命中的实际明细行文字标红；OCR 返回、人工新增/插入/粘贴/删除/修改单元格后都会自动回刷，同时同步刷新两个拣货单的 `客商编码` 只读状态与标签样式
...
- 「查询医疗器械」右侧新增「查询客商」入口，打开单实例、非模态二级列表窗口并立即调用 `QUERYCO`；请求头固定 `warehouseId=WH004078`、`customerId=GEHC`、`customerType=CO`；重复点击只聚焦已有窗口并刷新当前回填目标和目标说明，关闭后再次打开会重新查询
...
- 「查询医疗器械」「查询客商」「查询下单方」底部将查询状态与操作按钮分行显示；状态按窗口宽度自动换行，长错误不会挤压或遮挡右侧按钮
...
- 顶部按钮按「选择文件并开始处理 / 继续查询原任务 / 确认并导出 / 接口发送 / 中止」顺序排列，所有按钮按文字自然宽度显示，避免固定宽度造成截断或多余留白
...
4. 等待处理完成，界面日志会展示上传、OCR 提交、轮询和解析进度。若单个文件 5 分钟无结果，页签会显示 `结果未生成`；可点击顶部「继续查询原任务」继续等待，也可以直接人工填写该页签并导出。
5. 处理完成后，界面按文件显示预览页签，页签顶部展示该文件的一组单据头，下方展示明细；单据头以多列表单展示、可直接编辑，明细单元格可双击修改，或用「新增行 / 插入行 / 复制行 / 粘贴行 / 删除行」调整明细；复制行也可使用 `Ctrl/Cmd+C / Ctrl/Cmd+V` 快捷键。
...
### 2026-09-24
- [新增] `GE-发票单` 的 LPN/Serial 与 QTY 不匹配而保留的原始明细行在预览中标红，并在明细标题提示「包含异常明细，请注意」；异常状态支持编辑保留、删除清除和撤销恢复，不影响 Excel 导出与接口发送
- [修复] 「查询医疗器械」「查询客商」「查询下单方」底部状态与按钮分行显示，状态按窗口宽度自动换行，避免启动查询失败时错误信息遮挡操作按钮
...
- 2026-09-15: [变更] 新增产品勾选医疗器械后自动启用并锁定效期控制，序列号控制和批次控制必须且只能选择一项；非医疗器械产品保持原有独立选择行为
...
- 2026-09-14: [新增] 程序启动后台查询全部医疗器械目录，HTTP 200 时覆盖用户主目录缓存，失败时回退上次缓存；新增医疗器械产品发送成功后自动回刷；缓存读取与覆盖兼容 macOS、Windows、麒麟 Linux
- 2026-09-14: [新增] OCR 或人工编辑后的明细命中医疗器械 SKU 时，「明细」标题行居中加粗红色显示「该订单包含医疗器械（标红显示），请注意！」，命中明细行标红；新增、插入、粘贴、删除和单元格修改后自动回刷，并新增客户端查询、缓存、三模板匹配测试
- 2026-09-13: [变更] 恢复底部「复制行 / 粘贴行」按钮；将「继续查询原任务」移至顶部「选择文件并开始处理」右侧；主界面按钮改为自然宽度，顶部按钮内部水平留白及相邻按钮间距为 10px，底部按钮内部水平留白及相邻按钮间距为 15px
