...
- 「查询医疗器械」右侧新增「查询客商」入口，打开单实例、非模态二级列表窗口并立即调用 `QUERYCO`；重复点击只聚焦已有窗口并刷新当前回填目标和目标说明，关闭后再次打开会重新查询
- 客商窗口展示客商编码、客商名称、地址、联系人和联系人电话，顶部提供客商编码与客商名称两个本地实时模糊搜索框，两个条件同时满足才显示；底部提供「使用选中客商」「重新查询」「关闭」，重新查询期间保留当前列表和搜索内容、禁用查询按钮，失败保留最近一次成功列表，成功（包括空列表）整体覆盖
