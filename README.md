...
- `客商编码` 只能通过「查询客商」窗口回填，OCR 不识别也不自动回填；每个页签独立保留已回填值，医疗器械命中状态变化时恢复该值，目录刷新会同步重算所有已打开页签
...
- 「查询医疗器械」右侧新增「查询客商」入口，打开单实例、非模态二级列表窗口并立即调用 `QUERYCO`；重复点击只聚焦已有窗口，关闭后再次打开会重新查询
...
- 客商编码回填仅针对执行操作时选中的 ORACLE/OSCAR 页签，且该页签必须命中医疗器械；可双击客商行或点击「使用选中客商」，只将客商编码覆盖写入当前页签，成功后关闭客商窗口，不修改名称、地址、联系人或电话
