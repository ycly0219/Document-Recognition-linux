...
| `consigneeAddress1` | `Ship To Address`（W） | `收货地址`（AD） | 解析时把连续空白折为单个空格、去除首尾空白 |
| `notes` | 当前无来源省略 | `申请说明`（AE） | 原值 |
...
| `hedi13` | 无来源省略 | 不再发送 | ORACLE 不发送；OSCAR 不再映射 `供应商` |
| `hedi14` | 无来源省略 | 不再发送 | ORACLE 不发送；OSCAR 不再映射 `收货人` |
| `hedi15` | 无来源省略 | 不再发送 | ORACLE 不发送；OSCAR 不再映射 `收货人电话` |
| `userDefine1..5` | `udf01..udf05`（Y-AC） | `udf01..udf05`（AF-AJ） | 列位置映射；ORACLE `udf01` 当前来自 `SHIP TO NO`，其余当前为空 |
...
| `sku` | `Item Number`（AG） | `物料编号`（AN） | 原值 |
| `qtyOrdered` | `Qty`（AO） | `数量`（AV） | 原值 |
| `packUom` | 固定 `EA`（AP） | 固定 `EA`（AW） | `PACK_UOM` |
| `price` | 固定 `0`（AV） | 固定 `0`（BC） | `PRICE` |
| `lotAtt04` | `Lot`（AI） | 无来源省略 | 原值 |
| `lotAtt05` | 固定 `ORACLE`（AJ） | 固定 `OSCAR`（AQ） | 货物来源 |
| `lotAtt07` | `Org`（AK） | `仓库`（AR） | 原值 |
| `lotAtt08` | `Pick From Subinv`（AL） | 状态（AS） | ORACLE 按 `GD/BAD` 后缀转 `GOOD/BAD`；OSCAR `好件` 转 `GOOD`，其余留空 |
| `lotAtt09` | `Serial`（AM） | `序列号`（AT） | 原值；空或 `N/A` 归一化为空并省略 |
| `lotAtt11` | `LPN`（AN） | 无来源省略 | 原值 |
| `dedi01` | `Task Id`（AW） | 无来源省略 | 原值 |
| `dedi02` | 无来源省略 | `跟踪号`（BE） | 原值 |
| `dedi03` | `Pick From Locator`（AY） | `货位`（BF） | 原值 |
...
- 头部无来源字段：`priority`、`expectedShipmentTime1`、`requiredDeliveryTime`、`consigneeAddress2/3`、收货人国家/省/市/区/街道/邮箱/电话/邮编、承运商、开单方、结算方、`channel`、`hedi09/10`、`invoicePrintFlag`、`route`、`stop`、`createSource`、`transportation` 均不发送。
